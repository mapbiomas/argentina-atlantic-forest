// ============================================================
// Atlantic Forest (Bosque Atlántico) | Collection 6 | Step 14 — Stable Map, Stable Points & Complement Classification (Round 4)
// ============================================================
//
// DESCRIPTION:
//   Round-4 equivalent of 11-stable_map.js: gap-fills the per-year
//   Round-4 preclassification (step 13 output), derives a stable
//   reference map for classes 48/65 from per-class frequency, draws
//   stable points, and reclassifies per year, masking every year's
//   reclassification to Round 2's class-36 pixels (same base as Round
//   3's step 11, since Rounds 3 and 4 are sibling branches).
//
// METHODOLOGY: identical to 11-stable_map.js (Round 3), with classes
// 48/65 instead of 15/19, and masked against class 36 instead of 14.
//
// INPUT:
//   - Round 4 preclassification (13-preclassification.js output).
...
//     `<region_name>stable_map_v<version>-col_6_<yearini>_clas3_yerbate`.
//   - Stable points. Exported as
//     `<region_name>-stable_points_v<version>-col_6_<yearini>_clas3_yerbate`.
//   - Complement classification (multi-year, masked to Round 2's class
//     36). Exported as
//     `BAstep_04_<region_name>-col_6_<yearini>_class_v<version>_clas3_yerbate`.
//
// NOTE (kept as in the original): the complement-classification export
// (SECTION 10) prepends an extra literal `"BA"` to both the description
// and the asset id, ahead of `region_name` and `stage_out` respectively
// — a pattern not present in Rounds 1–3's equivalent exports. Kept
// exactly as found, not reconciled.
//
...
//
// PREVIOUS STEP: 13-preclassification.js (produces the per-year
//                classification this script stabilizes)
// NEXT STEP:     15a-merge.js
//
...
// 🔁 REPLACE: Round-4 preclassification (13-preclassification.js output).
var class_map = ee.Image('projects/YOUR-PROJECT/assets/LAND-COVER/COLLECTION-3/GENERAL/CLASSIFICATION/PRECLASSIFICATION/BA/step_0' + stage_in2 + '-class_reg_' + reg2 + "-col_6_" + yearini + '-class_v' + version_class_in2 + "_yerbate")
    .clip(myRegion);
...
Export.image.toAsset({
    ...
    "description": region_name + '-' + 'stable_map_v' + version_stable_out + "Collection_6_" + yearini + "_clas3_yerbate",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `outputStable` above).
    "assetId": outputStable + '/' + region_name + 'stable_map_v' + version_stable_out + "-col_6_" + yearini + "_clas3_yerbate",
    "scale": 30,
...
Export.table.toAsset({
    ...
    "description": region_name + '-stable_points_v' + version_stable_out + "Collection_6_" + yearini + "_clas3_yerbate",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `outputAsset` above).
    "assetId": outputAsset + '/' + region_name + '-stable_points_v' + version_stable_out + "-col_6_" + yearini + "_clas3_yerbate"
});


// ============================================================
// SECTION 9 — PER-YEAR RECLASSIFICATION (masked to Round 2's class 36)
// ============================================================
var nTrees = 250;
...
for (var year_id3 in year_list) {
    ...
    var clasif_otros = clasif_base.select('classification_' + year3).eq(36);
...
// ============================================================
// NOTE (kept as in the original): the description/assetId prepend an
// extra literal "BA" not used in Rounds 1-3's equivalent exports.
Export.image.toAsset({
    "image": classifiedStack.toInt8(),
    "description": "BA" + region_name + '-' + 'col_6_' + yearini + '_class_v' + version_stable_out + "_clas3_yerbate",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `dir_pre_class` above).
    "assetId": dir_pre_class + "BA" + stage_out + '_' + region_name + '-' + 'col_6_' + yearini + '_class_v' + version_stable_out + "_clas3_yerbate",
    "scale": 30,
    "pyramidingPolicy": { '.default': 'mode' },
    "maxPixels": 1e13,
    "region": myRegion
