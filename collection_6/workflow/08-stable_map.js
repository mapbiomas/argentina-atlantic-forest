// ============================================================
// Atlantic Forest (Bosque Atlántico) | Collection 6 | Step 08 — Stable Map, Stable Points & Complement Classification (Round 2)
// ============================================================
//
// DESCRIPTION:
//   Round-2 equivalent of 04-stable_map.js: gap-fills the per-year
//   Round-2 preclassification (step 07 output), derives a stable
//   reference map for classes 11/12/14/36 from per-class frequency,
//   draws stable points, and reclassifies per year — this time also
//   masking every year's reclassification to Round 1's class-10 pixels,
//   same as step 07.
//
// METHODOLOGY: identical to 04-stable_map.js (SECTIONS 4-9), with two
// differences:
//   1. No forest-cover "consensus" mask (not applicable — Round 2 has no
//      forest class).
//   2. SECTION 9's per-year reclassification is masked to Round 1's
//      class 10, same masking pattern as step 07.
//
// INPUT:
//   - Round 2 preclassification (07-preclassification.js output).
//   - Round 1's final classification (06g output), used as a mask.
//   - Zones FeatureCollection (Atlantic Forest AR-PY regions).
//   - Annual Landsat mosaics for Argentina.
//
// OUTPUT:
//   - Stable map. Exported as
//     `<region_name>stable_map_v<version>-col_6_<yearini>_clas2`.
//   - Stable points. Exported as
//     `<region_name>-stable_points_v<version>-col_6_<yearini>_clas2`.
//   - Complement classification (multi-year, masked to Round 1's class
//     10). Exported as
//     `step_04_<region_name>-col_6_<yearini>_class_v<version>_clas2`.
//
...
//
// PREVIOUS STEP: 07-preclassification.js (produces the per-year
//                classification this script stabilizes)
// NEXT STEP:     09a-merge.js
//
...
// 🔁 REPLACE: Round-2 preclassification (07-preclassification.js output).
...
var classFrequency = { "11": freq, "12": freq, "14": freq, "36": freq };
...
var mascara_freq_mapref = base
    ...
    .where(referenceMap.eq(14), 14)
    .where(referenceMap.eq(36), 36)
    .rename("reference")
    .selfMask();
...
Export.image.toAsset({
    ...
    "description": region_name + '-' + 'stable_map_v' + version_stable_out + "Collection_6_" + yearini + "_clas2",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `outputStable` above).
    "assetId": outputStable + '/' + region_name + 'stable_map_v' + version_stable_out + "-col_6_" + yearini + "_clas2",
    "scale": 30,
...
var num_train_14 = ee.Number(nSamplesMax * percent_14 / 100).round().int16().max(nSamplesMin);
...
var training_pt = stables_areas_filter.stratifiedSample({
    ...
    classValues: [11, 12, 14, 36],
    classPoints: [num_train_11, num_train_12, num_train_14, num_train_36]
});
...
Export.table.toAsset({
    ...
    "description": region_name + '-stable_points_v' + version_stable_out + "Collection_6_" + yearini + "_clas2",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `outputAsset` above).
    "assetId": outputAsset + '/' + region_name + '-stable_points_v' + version_stable_out + "-col_6_" + yearini + "_clas2"
});


// ============================================================
// SECTION 9 — PER-YEAR RECLASSIFICATION (masked to Round 1's class 10)
// ============================================================
var nTrees = 250;
...
for (var year_id3 in year_list) {
    ...
    var clasif_otros = clasif_base.select('classification_' + year3).eq(10);
...
Export.image.toAsset({
    ...
    "description": region_name + '-' + 'col_6_' + yearini + '_class_v' + version_stable_out + "_clas2",
    // 🔁 REPLACE: Update to your own GEE asset folder (see
    // `dir_pre_class` above).
    "assetId": dir_pre_class + stage_out + '_' + region_name + '-' + 'col_6_' + yearini + '_class_v' + version_stable_out + "_clas2",
    "scale": 30,
    "pyramidingPolicy": { '.default': 'mode' },
    "maxPixels": 1e13,
    "region": myRegion
